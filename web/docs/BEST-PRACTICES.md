...
## Cloudflare R2 — only production assets

Full SSOT: **`docs/PRODUCTION_CLOUD_STORAGE.md`**

Ship gate: `node scripts/ship-production-to-r2.mjs` (inspect mesh/texture → labels → R2 → D1 SQL).  
Catalog: `web/api/production-catalog.json`. CDN host: `assets.grudge-studio.com`.

---

## Projectiles + combat VFX

Full SSOT: **`docs/PROJECTILES_AND_VFX.md`** · browser: Kind=`projectile` · fleet-needs track `combat`.

| Rule | Detail |
|------|--------|
| Subtype SI | arrow 0.45–1.0 m · bullet ≪ 0.08 m · cannonball 0.12–0.55 m · explosive 0.08–0.45 m |
| Never | Fit projectiles/VFX to 1.8 m human height |
| Runtime | Object pool · damage on **impact** · CCD · layer Projectile |
| Parry | arrow / bullet / orb only — not grenade / bomb / nova |
| Deploy | R2 + D1 + `assetUrl` — same pattern as all fleet binaries |
...
5. Category-blind “fit everything to 1.8 m” (weapons, arrows, buildings)  
6. Embedded Mixamo mesh instead of clips on grudge6  
7. Marking done without production score / smoke  
8. Local-only combat meshes with CDN `assetUrl` (prod 404)  
9. Per-shot mesh alloc instead of projectile pools  
